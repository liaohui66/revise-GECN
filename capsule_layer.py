...
class SE3CapsuleLayer(nn.Module):
    ...
    def forward(self, input_poses_algebra: torch.Tensor, input_activations: torch.Tensor):
        ...
        device_type = input_poses_algebra.device.type
        with torch.amp.autocast(device_type=device_type, enabled=False):
            # 1. 将所有可能为 float16 的输入张量，显式地转换为 float32
            input_poses_f32 = input_poses_algebra.to(torch.float32)
            input_activations_f32 = input_activations.to(torch.float32)
            
            # 2. 在这个 float32 的“安全区”内，执行你原有的全部 forward 逻辑
            #    我们只需将所有输入变量替换为新创建的 _f32 版本
            B = input_poses_f32.shape[0]
            if input_activations_f32.ndim == 2:
                input_activations_f32 = input_activations_f32.unsqueeze(-1)
            
            if self.se3_transforms_tij_algebra is None:
                raise NotImplementedError("Dynamic transform generation is not implemented.")
            
            # 确保变换矩阵也是 float32
            current_t_ij_algebra = self.se3_transforms_tij_algebra.to(torch.float32)
            
            # _apply_se3_transform_vote_algebra 内部的计算现在都是 float32
            votes_algebra = self._apply_se3_transform_vote_algebra(input_poses_f32, current_t_ij_algebra)

            # Initial pose proposal
            initial_routing_weights_expanded = input_activations_f32.unsqueeze(1).expand(
            ...
            # Iterative Routing (现在完全在 float32 下进行)
            for _ in range(self.num_iterations):
                ...
                # 使用 float32 的激活值
                routing_weights_w = routing_weights_w_unnormalized * input_activations_f32.transpose(1,2)
            ...
            # --- Final activation calculation (现在完全在 float32 下进行) ---
