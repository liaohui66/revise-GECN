...
class SE3CovariantOutputLayer(torch.nn.Module):
    ...
    def forward(self, aggregated_equivariant_feature_tensor: torch.Tensor) -> torch.Tensor:
        device_type = aggregated_equivariant_feature_tensor.device.type
        with torch.amp.autocast(device_type=device_type, enabled=False):
            feature_f32 = aggregated_equivariant_feature_tensor.to(torch.float32)
            tensor_basis_features = self.projection_to_tensor_irreps(feature_f32)
